/**
 * 慧惠 AI 聊天组件 — Cloudflare Pages Functions 代理
 * 接收前端请求 → 组装 System Prompt → 转发 DeepSeek API → 返回答案
 * API Key 存储在 Cloudflare 环境变量 DEEPSEEK_API_KEY 中
 */
...
export async function onRequest(context) {
    ...
    try {
        ...
        const userLevel = level || 'L2';  // 默认 L2 精读
        ...
        if (!resp.ok) {
            ...
            return new Response(JSON.stringify({ error: 'DeepSeek API error: ' + resp.status, detail: errData }), {
                status: resp.status,
