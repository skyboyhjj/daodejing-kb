/**
 * 慧惠 API Worker — 独立 Cloudflare Worker
 * 替代 Pages Functions，处理 /api/chat 请求
 * 
 * 部署方式: Cloudflare Dashboard → Workers & Pages → Create → Create Worker
 * 环境变量: DEEPSEEK_API_KEY (加密存储)
 */
export default {
    async fetch(request, env, ctx) {
        ...
        if (!env.DEEPSEEK_API_KEY) {
            return new Response(JSON.stringify({
                error: 'DEEPSEEK_API_KEY 环境变量未配置。请在 Worker 设置中添加该变量。'
            }), {
                status: 500,
                headers: {
                    'Content-Type': 'application/json',
