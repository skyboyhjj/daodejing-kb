...
PREDICATE_MAP = {
    ...
    "处于": "处", "接近": "几", "如同": "若",
    # 否定式固定短语（4 → 不X，统一承载"不争、不言、不可、不如"等）
    "不与之争": "不X", "不如": "不X", "不可": "不X", "不言": "不X",
...
def main():
    ...
    # G7 映射表防膨胀红线（v1.3）：谓词总数硬上限 35 条
    map_count = len(PREDICATE_MAP)
    print(f"[G7 映射红线] 谓词映射 {map_count}/35 条" + (" ⚠ 接近上限" if map_count >= 30 else " ✅"))
    if map_count > 35:
        print("[G7 违规] 谓词映射超 35 条上限！启动谓词归并，禁止继续新增", file=sys.stderr)
